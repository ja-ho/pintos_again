...
//my func
void memory_calc(char argv[][64], int argv_count)
{
	int i;
	void *phys_basis = 0xc0000000;
	int memory=0;
	for(i=0; i<argv_count; i++) {
		memory += strlen(argv[i]);
	}
	memory += 4 - (memory % 4);
	memory += 4*(argv_count+1);
	memory += 12;
	printf("%p\n", phys_basis - memory);
}
//
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  process_activate ();
	...
	strlcpy(copy_file_name, token, strlen(token)+1);
...
setup_stack (void **esp, const char *file_name) 
{
  ...
  bool success = false;
	...
	int argv_count = 0;
	int i;
	unsigned int word_align_address;
	//tokenize
	...
	for (token = strtok_r (copy_name, " ", &save_ptr); token != NULL; token = strtok_r (NULL, " ", &save_ptr)) {
		//printf("token: %s\n", token);
		strlcpy(argv[argv_count], token, strlen(token) + 1);
		argv_count++;
	}

	//memory_calc(argv, argv_count);
	//
  ...
  if (kpage != NULL) 
	...
	address_memory = (void *)malloc(sizeof(void *)*argv_count+1);
	//my implement argument passing
	for(i=argv_count - 1; i>=0; i--) {
		*esp -= strlen(argv[i])+1;
		address_memory[i] = *esp;
		memcpy(*esp, argv[i], strlen(argv[i])+1);
	}
	word_align_address = *esp;
	*esp -= (word_align_address%4);	//word align

	//*esp -= sizeof(char *);	//last array index
	address_memory[argv_count] = 0;
	for(i=argv_count; i>=0; i--) {
		*esp -= sizeof(void *);
		memcpy(*esp, &address_memory[i], sizeof(void *));
	}
	argv_address = *esp -4;
	*esp -= sizeof(char *);
	memcpy(*esp, argv_address, sizeof(void *));
	*esp -= sizeof(int);
	memset(*esp, argv_count, 1);
	*esp -= sizeof(void *);
	memset(*esp, 0, sizeof(void *));

	hex_dump((unsigned int)*esp, *esp, 0x50, true); 
	free(address_memory);
	//
  return success;
}
